...
import org.netsharp.panda.annotation.HtmlPropertyAnnotation;
...
public class Control {
	...
	protected static final Log logger = LogFactory.getLog(Control.class.getName());
...
//		String spaces = this.getSpaces();
	...
	protected String renderProperties() {
		...
		for (DataOptionAnnotation att : map.DataOptionProperties) {
			...
			if(str==null){
				logger.debug(this.getClass().getName()+"["+att.html+"] is null");
			}
	...
	private String PropertyValue(HtmlPropertyAnnotation att) {
		if (att.serializeType != IControlSerializer.class) {
			IControlSerializer serizer = (IControlSerializer) ReflectManager.newInstance(att.serializeType);
