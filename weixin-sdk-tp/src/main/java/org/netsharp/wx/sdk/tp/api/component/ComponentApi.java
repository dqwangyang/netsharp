package org.netsharp.wx.sdk.tp.api.component;
...
import org.netsharp.wx.sdk.tp.bean.response.component.*;
import org.netsharp.wx.sdk.tp.utils.Params;
import org.netsharp.wx.sdk.tp.utils.UrlUtils;
...
public class ComponentApi {
    ...
    public String getComponentLoginPageTD(String componentAppId, String preAuthCode,
                                        String redirectUri,String bizAppid) {
        return Const.Uri.MP_URI + "/cgi-bin/componentloginpage?component_appid=" + componentAppId
                + "&pre_auth_code=" + preAuthCode + "&redirect_uri=" + UrlUtils.encode(redirectUri)
                + "&biz_appid=" + bizAppid;
    }
     ...
     * @param code  授权code,会在授权成功时返回给第三方平台，详见第三方平台授权流程说明
     */
    public QueryAuthMiniResponse queryAuthMini(String token, String appid, String code) {
        String data = String.format("{\"component_appid\":\"%1$s\",\"authorization_code\":\"%2$s\"}", appid, code);
        Map<String, String> params = Params.create("component_access_token", token).get();
        return HttpUtil.postJsonBean(COMPONENT_API_QUERY_AUTH, params, data, QueryAuthMiniResponse.class);
    }
