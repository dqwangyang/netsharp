package org.netsharp.wx.sdk.tp.utils.client.httpclient;
...
import org.netsharp.wx.sdk.tp.utils.client.HttpDelegate;
...
public class HttpClientDelegate implements HttpDelegate {
    ...
    @Override
    public MediaFile download(String url, Map<String, String> params,String postData) {
        MediaFile response = null;
        try {
            response = httpClient.execute(buildPostRequest(url, params,postData), fileHandler);
        } catch (Exception e) {
            LOGGER.error("Download error.url: " + url + ", params: " + params, e);
        }
        return response;
            ...
            if (status >= 200 && status < 300) {
                ...
                if (contentType.equalsIgnoreCase("text/plain")) {
                    ...
                    if(contentType.contains("image")){
                        try {
                            mediaFile.setBufferedImage(ImageIO.read(response.getEntity().getContent()));
                        } catch (IOException e) {
                            e.printStackTrace();
